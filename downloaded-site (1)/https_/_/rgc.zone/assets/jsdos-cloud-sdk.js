...
var CloudSDK;
(function (CloudSDK) {
    const rgcProxy = "/jsdos-cloud-proxy.php";
    async function resolveToken(token) {
        if (token && token.length === 5) {
            const response = await fetch(rgcProxy + "?op=token&id=" + encodeURIComponent(token));
    ...
    async function pushToStorage(token, fileName, payload) {
        ...
        const presign = await (await fetch(rgcProxy + "?op=presign&path=" +
            encodeURIComponent(fileName) + "&token=" + encodeURIComponent(token))).json();
        if (!presign.success) {
        ...
        formData.append("__s3url", post.url);
        const response = await fetch(rgcProxy + "?op=push", {
            method: "post",
            body: formData,
        });
    ...
    async function pullFromStorage(token, fileName) {
        ...
        if (profile && profile.email) {
            const response = (await fetch(rgcProxy + "?op=pull&email=" +
                encodeURIComponent(profile.email) + "&file=" + encodeURIComponent(fileName), {
                cache: "no-cache",
            }));
...
(function (CloudSDKUI) {
    ...
    function NoKey() {
        ...
        function hideIfIdle() {
            ...
            const input = document.querySelector("#cloud-saves .key-input");
            if (input && (document.activeElement === input || input.value.length > 0)) {
                setTimeout(hideIfIdle, 18000);
                return;
            }
            state = "hidden";
        }
        return {
            ...
            <div class="cloud-saves-no-key">
                <button type="button" class="cloud-saves-clear" aria-label="close">
            ...
            bind: (root) => {
                ...
                const close = root.querySelector(".cloud-saves-clear");
            ...
            unbind: (root) => {
                ...
                const close = root.querySelector(".cloud-saves-clear");
                if (close) close.removeEventListener("click", hide);
            },
        };
    }
    function Key() {
        ...
        return {
            ...
            bind: (root) => {
                var _a;
                (_a = root.querySelector(".cloud-saves-clear")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", hide);
                setTimeout(() => {
                    if (state === "key") {
                        state = "hidden";
                    }
            ...
            unbind: (root) => {
                var _a;
                (_a = root.querySelector(".cloud-saves-clear")) === null || _a === void 0 ? void 0 : _a.removeEventListener("click", hide);
            },
        };
    }
    function Hidden() {
    ...
    const cssStyles = `
        ...
        .cloud-saves-no-key .cloud-saves-clear {
            position: absolute;
            top: -6px;
            right: -10px;
        }
